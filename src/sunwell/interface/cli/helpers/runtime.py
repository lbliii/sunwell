...
def check_free_threading(quiet: bool = False) -> bool:
    ...
    if not quiet and os.environ.get("SUNWELL_NO_GIL_WARNING") != "1":
        ...
        stderr_console.print("[yellow]⚠️  Running on standard Python (GIL enabled)[/yellow]")
        stderr_console.print(
            "[dim]   For optimal performance, use Python 3.14t (free-threaded):[/dim]"
        )
        stderr_console.print("[dim]   /usr/local/bin/python3.14t -m sunwell chat[/dim]")
        stderr_console.print("[dim]   Set SUNWELL_NO_GIL_WARNING=1 to suppress this message.[/dim]")
        stderr_console.print()
