...
class ChannelRouter:
    ...
    fallback: bool = True
    parallel: bool = False
    _channels: list[tuple[int, NotificationChannel]] = field(default_factory=list, init=False)
    ...
    async def _send_sequential(
        ...
        for priority, channel in channels:
            try:
                success = await channel.send(title, message, notification_type, context=context)
                if success:
                    logger.debug(
                    ...
                    logger.debug(f"Channel {channel.config.name} failed, trying next...")
