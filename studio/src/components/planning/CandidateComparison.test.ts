...
describe('CandidateComparison', () => {
  ...
  it('filters out undefined entries from sparse arrays', () => {
    ...
    const sparseArray = createSparseArray(6, {
      0: mockCandidate,
      5: { ...mockCandidate, id: 'candidate-5', index: 5, artifact_count: 10 },
    });

    const { getByText, queryByText } = render(CandidateComparison, {
  ...
  it('highlights selected candidate by id', () => {
    const candidates: PlanCandidate[] = [
      mockCandidate,
      { ...mockCandidate, id: 'candidate-1', index: 1, artifact_count: 8 },
    ];
