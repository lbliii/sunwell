...
class TestSkillAwareClassifier:
    """Tests for skill-aware task classification."""

    def test_explicit_skill_mention(self):
        """Task explicitly mentioning skill name should match."""
        from sunwell.skills.executor import SkillAwareClassifier

        loader = LensLoader()
        lens = loader.load(Path("lenses/tech-writer.lens"))
        classifier = SkillAwareClassifier(lens=lens)

        result = classifier.classify("Use create-api-docs to document my code")
        assert result["skill"] is not None
        assert result["skill"].name == "create-api-docs"
        assert result["confidence"] >= 0.9

    def test_implicit_match_api_docs(self):
        """Task about API documentation should match an API-related skill."""
        from sunwell.skills.executor import SkillAwareClassifier
    ...
    def test_implicit_match_readme(self):
        ...
        result = classifier.classify("Generate a README for this project")
        assert result["skill"] is not None
        assert result["skill"].name == "create-readme"
        assert result["confidence"] >= 0.6

    def test_implicit_match_tutorial(self):
        """Task about tutorial should match a learning-oriented skill."""
        from sunwell.skills.executor import SkillAwareClassifier
    ...
    def test_no_skill_match(self):
        """General tasks should not match any skill."""
        from sunwell.skills.executor import SkillAwareClassifier
        ...
        result = classifier.classify("Review this document for clarity")
        assert result["skill"] is None
        assert result["reason"] == "No skill needed"

    def test_lens_without_skills(self):
        """Lens without skills should return no match."""
        from sunwell.skills.executor import SkillAwareClassifier
        from sunwell.core.lens import Lens, LensMetadata

        # Create a minimal lens without skills
        lens = Lens(metadata=LensMetadata(name="Empty Lens"))
        classifier = SkillAwareClassifier(lens=lens)

        result = classifier.classify("Create API docs")
        assert result["skill"] is None
        assert "No skills available" in result["reason"]
