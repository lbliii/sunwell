...
**Target Version**: v1.x  
**Confidence**: 90% 🟢  
**Depends On**: RFC-096 (Project Manager)  
**Related**: RFC-095 (Demo Command — quick 2-min proof, this RFC is rigorous evaluation)  
**Evidence**:
- `src/sunwell/demo/executor.py` — DemoExecutor pattern for comparison
- `src/sunwell/naaru/resonance.py` — Resonance feedback loop
- `src/sunwell/naaru/coordinator.py:197` — `Naaru.process()` entry point
- `src/sunwell/types/naaru_api.py:87` — `ProcessInput` contract
- `src/sunwell/types/naaru_api.py:30` — `NaaruEventType` enum
- `src/sunwell/surface/lens_detection.py:64` — `get_lens_for_project()`
- `src/sunwell/incremental/cache.py:74` — `ExecutionCache` SQLite pattern
- `src/sunwell/security/approval_cache.py:232` — `SQLiteApprovalCache` pattern
- `examples/forum_app/` — Reference implementation
...
| Aspect | `sunwell demo` (RFC-095) | `sunwell eval` (this RFC) |
|--------|--------------------------|---------------------------|
| **Purpose** | "Holy shit" moment | Rigorous quality measurement |
| **Time** | ~2 minutes | 10-30 minutes |
| **Tasks** | Simple (single function) | Complex (multi-file projects) |
| **Runs** | 1 | 3+ (statistical validity) |
| **Storage** | None | SQLite with history |
| **CI/CD** | No | Yes (JUnit XML, exit codes) |
| **Regression** | No | Yes (baseline comparison) |
| **Use case** | Marketing, onboarding | Development, optimization |
...
from sunwell.types.naaru_api import NaaruEventType
...
class SunwellFullStackExecutor:
    ...
    async def run(
        ...
        *,
        ...
        # Auto-detect lens if not specified (uses existing lens detection)
        lens = self.lens_name or get_lens_for_project(output_dir)
        
        # Build Naaru input using existing ProcessInput contract
        # Lens and tools passed via context dict (the extensible field)
        eval_input = ProcessInput(
            content=task.prompt,
            ...
            context={
                ...
                "evaluation_mode": True,  # Signal this is an evaluation run
            },
        )
        ...
        # Map existing NaaruEventType values to evaluation semantics
        async for event in self.naaru.process(eval_input):
            match event.type:
                # File creation comes through TOOL_RESULT when create_file tool is called
                case NaaruEventType.TOOL_RESULT:
                    if event.data.get("tool") == "create_file":
                        files_created.append(event.data["path"])
                        if on_file_created:
                            on_file_created(event.data["path"])
                
                # Judge results come through VALIDATION_RESULT
                case NaaruEventType.VALIDATION_RESULT:
                    score = event.data.get("score", 0.0)
                    judge_scores.append(score)
                    if on_judge:
                        on_judge(score, event.data.get("issues", []))
                    # Resonance is triggered when validation fails
                    if not event.data.get("valid", False):
                        resonance_count += 1
                        if on_resonance:
                            on_resonance(resonance_count)
                
                # Token tracking via MODEL_TOKENS
                case NaaruEventType.MODEL_TOKENS:
                    total_tokens += len(event.data.get("tokens", ""))
...
> **API Note**: This implementation uses the existing `ProcessInput` contract from `types/naaru_api.py`.
> Evaluation-specific parameters (`lens`, `tools`, `evaluation_mode`) are passed via the extensible
> `context` dict field. Event mapping uses existing `NaaruEventType` values:
> - `TOOL_RESULT` → file creation (when `tool == "create_file"`)
> - `VALIDATION_RESULT` → judge scores and resonance triggers
> - `MODEL_TOKENS` → token tracking
...
## API Contract: Naaru Integration

This section documents how evaluation concepts map to existing Naaru infrastructure.

### ProcessInput Contract

The evaluation framework uses the existing `ProcessInput` type from `types/naaru_api.py:87`:
...
class ProcessInput:
    ...
    content: str                              # The task prompt
    mode: ProcessMode = ProcessMode.AUTO      # Use ProcessMode.AGENT for evaluation
    workspace: Path | None = None             # Output directory
    context: dict[str, Any] = field(...)      # Extension point for evaluation params
```

**Evaluation-specific context fields:**

| Field | Type | Purpose |
|-------|------|---------|
| `lens` | `str \| None` | Lens name to use (e.g., `"coder.lens"`) |
| `tools` | `list[str]` | Available tools for the task |
| `evaluation_mode` | `bool` | Signal that this is an evaluation run |

### NaaruEventType Mapping

The evaluation framework maps existing `NaaruEventType` values to evaluation semantics:

| Evaluation Concept | NaaruEventType | How to detect |
|--------------------|---------------|---------------|
| File created | `TOOL_RESULT` | `event.data["tool"] == "create_file"` |
| Judge score | `VALIDATION_RESULT` | `event.data["score"]` |
| Resonance iteration | `VALIDATION_RESULT` | `event.data["valid"] == False` triggers refinement |
| Token usage | `MODEL_TOKENS` | `len(event.data["tokens"])` |

**Note**: If evaluation-specific events are needed in the future, they can be added to `NaaruEventType`:
- `EVAL_FILE_CREATED` — explicit file creation during evaluation
- `EVAL_JUDGE_SCORE` — judge score with full rubric
- `EVAL_RESONANCE_START` / `EVAL_RESONANCE_END` — refinement boundaries

For v1, we use the existing event types to minimize API changes.

### Lens Detection

Uses the existing lens detection from `surface/lens_detection.py`:

```python
from sunwell.surface.lens_detection import get_lens_for_project

# Auto-detect appropriate lens from project structure
lens = get_lens_for_project(output_dir)
```
...
- [ ] `SunwellExecutor` **routes through `Naaru.process()`** — dogfoods unified API
  - Uses `ProcessInput` with evaluation params in `context` dict
  - Maps `NaaruEventType` to evaluation events (see API Contract section)
  - Uses `get_lens_for_project()` from `surface/lens_detection.py`
...
| **Lens** | `surface/lens_detection.py` | Auto-detect lens via `get_lens_for_project()` |
...
1. **`SunwellFullStackExecutor` MUST use `Naaru.process()`**:
   ...
   async def run(self, task: FullStackTask, ...) -> SunwellResult:
       ...
       async for event in self.naaru.process(ProcessInput(
           ...
           # Handle events via existing NaaruEventType mappings:
           # - TOOL_RESULT → file creation
           # - VALIDATION_RESULT → judge scores
           # - MODEL_TOKENS → token tracking
           ...
   ```
...
| Question | Decision | Rationale |
|----------|----------|-----------|
| **ProcessInput extension** | Use `context` dict, not new fields | `ProcessInput.context` is the designed extension point. Adding `lens`/`tools` fields would break API. |
| **NaaruEventType extension** | Reuse existing events for v1 | `TOOL_RESULT`, `VALIDATION_RESULT`, `MODEL_TOKENS` cover our needs. Avoids API churn. |
| **Lens detection** | Use `get_lens_for_project()` | Existing function in `surface/lens_detection.py`. No new lens API needed. |
...
| **Evaluation-specific events** | Reuse existing events for v1. Add dedicated events if usage patterns require. |
...
- RFC-095: Demo Command (quick 2-minute proof-of-concept — separate from this)
- RFC-096: Project Manager (project orchestration)
- RFC-083: Naaru Unified API (used by `SunwellExecutor`)
